...
export class PipelineService {
  ...
  async run(
    ...
    try {
      for (let i = 0; i < uniquePages.length; i++) {
        ...
        onProgress?.({
          step: 'extracting', message: `提取 (${i + 1}/${uniquePages.length}): ${page.title || page.url}`,
          percent: 25 + Math.floor((i / Math.max(uniquePages.length, 1)) * 45),
        });
        const ex = await this.extract(page.textContent, page.url, config);
  ...
  private async extract(text: string, sourceUrl: string, config: PipelineConfig): Promise<{
    ...
    if (!apiKey || apiKey === 'sk-local' || apiKey === 'sk-your-api-key-here') {
...
只返回JSON数组，不要其他内容。文本：\n\n${text.substring(0, 8000)}`;
      ...
      if (arrStart === -1 || arrEnd === -1) return this.heuristicExtract(text, sourceUrl);
      const entities: any[] = JSON.parse(entityRaw.substring(arrStart, arrEnd + 1));
...
只返回JSON数组，不要其他内容。文本：\n\n${text.substring(0, 6000)}`;
      ...
      if (rStart !== -1 && rEnd !== -1) {
        try { relationships = JSON.parse(relRaw.substring(rStart, rEnd + 1)); } catch { /* ignore parse errors */ }
      }
      return {
